...
import { useCategoriesList } from "../hooks/use-master";
...
export function CategoriesListPage() {
  ...
  const [sortField, setSortField] = useState<"product_category" | "product_sort" | null>(null);
  const [sortAsc, setSortAsc] = useState(true);

  const handleSort = (field: "product_category" | "product_sort") => {
    trigger("light");
    if (sortField === field) {
      setSortAsc(!sortAsc);
    } else {
  ...
  const filteredCategories = categories
    .filter((cat) => {
      const matchesSearch = cat.product_category.toLowerCase().includes(search.toLowerCase());
      const matchesStatus = statusFilter === "ALL" || 
        (cat.product_category_status && cat.product_category_status.toLowerCase() === statusFilter.toLowerCase());
    ...
    <div className="flex flex-col gap-5 p-4 md:p-6 w-full max-w-7xl mx-auto pb-24 md:pb-6 animate-fade-in duration-300">
      <PageHeader title="Master Data" subtitle="Configure categories, sub-categories, brands, and products.">
        <Button asChild className="shrink-0 cursor-pointer shadow-sm">
          <Link to="/add-categories" onClick={() => trigger("light")}>
          ...
          <CardContent className="p-0">
            {isLoading ? (
              <div className="h-64 flex flex-col items-center justify-center gap-3">
                ...
                <span className="text-sm font-semibold text-text-muted animate-pulse">Loading categories...</span>
              </div>
            ) : (
                  ...
                  <TableHeader className="bg-muted/40 border-b border-border/60">
                    <TableRow className="hover:bg-transparent">
                      ...
                      <TableHead className="py-3.5 px-4 font-bold text-xs uppercase tracking-wider text-text-muted w-32">
                        <Button
                          variant="ghost"
                          onClick={() => handleSort("product_sort")}
                          className="-ml-4 hover:bg-primary/5 hover:text-primary font-bold text-xs uppercase tracking-wider text-text-muted transition-colors duration-200"
                        >
                          Sort Order
                          <ArrowUpDown className="ml-1.5 size-3.5 shrink-0" />
                  ...
                  <TableBody>
                    {filteredCategories.length > 0 ? (
                      filteredCategories.map((category) => {
                        ...
                        const isActive = category.product_category_status === "Active";

                        return (
                          <TableRow key={category.id} className="border-b border-border/40 hover:bg-primary/[0.03] dark:hover:bg-primary/[0.08] transition-all duration-300 ease-in-out">
                                ...
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Button
                                      ...
                                      asChild
                                    >
                                      <Link to={`/edit-categories/${category.id}`} onClick={() => trigger("light")}>
                      ...
                      <TableRow>
                        <TableCell colSpan={5} className="h-48 text-center text-text-muted p-6">
                          <div className="flex flex-col items-center justify-center gap-2">
                            ...
                            <p className="font-semibold text-text/80">No categories found.</p>
                            <p className="text-xs text-text-muted max-w-xs">
                              Try searching for a different term or add a new category.
                            </p>
                          </div>
                        </TableCell>
                      </TableRow>
