...
import { useSubCategoriesList, useActiveCategories } from "../hooks/use-master";
...
export function SubCategoriesListPage() {
  ...
  const { data: subcategories = [], isLoading } = useSubCategoriesList();
  const { data: categories = [] } = useActiveCategories();
  const [search, setSearch] = useState("");
  const [categoryFilter, setCategoryFilter] = useState("ALL");
  const [statusFilter, setStatusFilter] = useState("ALL");
  const [sortField, setSortField] = useState<"product_category" | "product_sub_category" | null>(null);
  const [sortAsc, setSortAsc] = useState(true);

  const handleSort = (field: "product_category" | "product_sub_category") => {
    trigger("light");
    if (sortField === field) {
      setSortAsc(!sortAsc);
    } else {
  ...
  const filteredSubCategories = subcategories
    .filter((sub) => {
      const matchesSearch =
        sub.product_sub_category.toLowerCase().includes(search.toLowerCase()) ||
        (sub.product_category || "").toLowerCase().includes(search.toLowerCase());
      const matchesCategory = categoryFilter === "ALL" || 
        (sub.product_category || "").trim().toLowerCase() === categoryFilter.trim().toLowerCase();
      const matchesStatus = statusFilter === "ALL" || 
        (sub.product_sub_category_status && sub.product_sub_category_status.toLowerCase() === statusFilter.toLowerCase());
    ...
    <div className="flex flex-col gap-5 p-4 md:p-6 w-full max-w-7xl mx-auto pb-24 md:pb-6 animate-fade-in duration-300">
      <PageHeader title="Master Data" subtitle="Configure categories, sub-categories, brands, and products.">
        <Button asChild className="shrink-0 cursor-pointer shadow-sm">
          <Link to="/add-sub-categories" onClick={() => trigger("light")}>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-panel border border-border/80 rounded-2xl p-4 md:p-5 shadow-xs mb-1">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          <Select value={categoryFilter} onValueChange={(val) => { trigger("light"); setCategoryFilter(val); }}>
            <SelectTrigger className="w-full md:w-[180px] bg-background border-border rounded-xl h-10">
          ...
          <CardContent className="p-0">
            {isLoading ? (
              <div className="h-64 flex flex-col items-center justify-center gap-3">
                ...
                <span className="text-sm font-semibold text-text-muted animate-pulse">Loading sub categories...</span>
              </div>
            ) : (
                  ...
                  <TableBody>
                    {filteredSubCategories.length > 0 ? (
                      filteredSubCategories.map((subcategory) => {
                        ...
                        const isActive = subcategory.product_sub_category_status === "Active";

                        return (
                          <TableRow key={subcategory.id} className="border-b border-border/40 hover:bg-primary/[0.03] dark:hover:bg-primary/[0.08] transition-all duration-300 ease-in-out">
                            <TableCell className="py-3.5 px-4">
                              <img
                                src={imageUrl}
                                alt={subcategory.product_sub_category}
                                ...
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Button
                                      ...
                                      asChild
                                    >
                                      <Link to={`/edit-sub-categories/${subcategory.id}`} onClick={() => trigger("light")}>
                      ...
                      <TableRow>
                        <TableCell colSpan={5} className="h-48 text-center text-text-muted p-6">
                          <div className="flex flex-col items-center justify-center gap-2">
                            ...
                            <p className="font-semibold text-text/80">No sub categories found.</p>
                            <p className="text-xs text-text-muted max-w-xs">
                              Try searching for a different term or add a new sub category.
                            </p>
                          </div>
                        </TableCell>
                      </TableRow>
